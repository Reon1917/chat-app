...
-- Step 1: Drop all existing tables and functions in reverse dependency order
DO $$ 
BEGIN
  -- Drop realtime publication first
  DROP PUBLICATION IF EXISTS supabase_realtime;
  
  -- Drop triggers
  DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
  DROP TRIGGER IF EXISTS on_message_sent ON public.direct_messages;
  ...
  -- First drop all policies that cause circular dependencies
  DROP POLICY IF EXISTS "Users can view conversations they are in" ON public.direct_conversations;
  ...
  DROP POLICY IF EXISTS "Private rooms are viewable only by members" ON public.rooms;
  
  -- Drop tables in reverse dependency order to avoid constraint violations
  -- Using CASCADE option to ensure dependent objects are also dropped
